...
def bolt(f_name):
    ...
    #----------------------------------------------------------------------------------------------------------------------------
    # Bolt/stud lead screw dimension
    myfile_in.readline()
    Ds = float(myfile_in.readline().strip())
    ps = float(myfile_in.readline().strip())
    ...
    # Screw resistant diameter
    ds = ((Ds-0.64952*ps)+(Ds-1.22687*ps))/2
    #----------------------------------------------------------------------------------------------------------------------------
    ...
    beam_prop(name,x_axes[0],x_axes[1],x_axes[2],ds)
    #----------------------------------------------------------------------------------------------------------------------------
    ...
    n_temp = py_get_int("max_node_id()")
    e_temp = py_get_int("max_element_id()")     
    py_send("*prog_param expand:trans_z %f " %(-Ls/int(2*Ls/Ds)))
    py_send("*prog_param expand:repetitions %f" %(int(2*Ls/Ds)))
    py_send("*expand_nodes")
    py_send("%d" %(n_temp))
    py_send("# | End of List")
    py_send("*remove_unused_nodes")
    ...
    py_send("# | End of List")
    #----------------------------------------------------------------------------------------------------------------------------
    # Creation of a list of all RBE3 dependent node
    rbe3_list = []
    rbe3_list.append(py_get_int("max_node_id()"))    
    #----------------------------------------------------------------------------------------------------------------------------
    ...
    for i in range(0,len(center)):
        py_send("*new_rbe3")
        py_send("*rbe3_name rbe3_%s_%s" %(group_name,str(i+1).zfill(3)))
        py_send("*rbe3_ref_node %d" %rbe3_list[i])
        py_send("*set_rbe3_ref_dof x on")
        py_send("*set_rbe3_ref_dof y on")
        py_send("*set_rbe3_ref_dof z on")
        py_send("*set_rbe3_ref_dof rx on")
        py_send("*set_rbe3_ref_dof ry on")
        py_send("*set_rbe3_ref_dof rz on")
        py_send("*set_rbe3_ret_dof_def x")
        py_send("*set_rbe3_ret_dof_def y")
        py_send("*set_rbe3_ret_dof_def z")
        py_send("*prog_param rbe3:conn_coef_def 1")
    #----------------------------------------------------------------------------------------------------------------------------
    # Washer creation
    if rond_check == "True":
    #----------------------------------------------------------------------------------------------------------------------------
